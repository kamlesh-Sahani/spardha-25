  ...
  const toggleStatus = (id: number) => {
    setEvents((prevEvents) =>
      prevEvents.map((event) =>
        event.id === id
          ? { ...event, status: event.status === "Active" ? "Disabled" : "Active" }
          : event
      )
    );
  };
            ...
            {events.map((event) => (
              <TableRow key={event.id}>
                ...
                <TableCell>
                  ...
                  />
                  <span className={`ml-2 font-medium ${event.status === "Active" ? "text-green-600" : "text-red-600"}`}>
                    {event.status}
                  </span>
                </TableCell>
                <TableCell>
                  <Button onClick={() => openEventModal(event)}>View Details</Button>
                </TableCell>
              </TableRow>
            ))}
          </TableBody>
